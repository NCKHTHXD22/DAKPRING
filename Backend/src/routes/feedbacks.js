...
router.get('/', async (req, res) => {
  try {
    ...
    // Lọc theo quyền: officer/staff chỉ thấy phản ánh được phân công cho mình
    const me = req.user
    ...
    // Enrich displayName/avatar từ Redis profile cache cho những feedback chưa có
...
router.post('/:id/draft', requireRole('officer', 'staff'), async (req, res) => {
  try {
    ...
    // Thông báo in-app cho lãnh đạo phụ trách danh mục (hoặc không giới hạn danh mục)
