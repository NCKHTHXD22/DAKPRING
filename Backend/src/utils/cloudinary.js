...
async function uploadFromBuffer(buffer, filename) {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder: 'DAKPRING-goopy', resource_type: 'image', public_id: filename },
      (err, result) => {
...
async function uploadBufferGeneric(buffer, publicId, resourceType = 'image') {
  return new Promise((resolve, reject) => {
    const stream = cloudinary.uploader.upload_stream(
      { folder: 'DAKPRING-task-attachments', resource_type: resourceType, public_id: publicId },
      (err, result) => {
        if (err) return reject(err);
        resolve(result.secure_url);
      }
