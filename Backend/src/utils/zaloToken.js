...
async function refreshAccessToken() {
  ...
  await redisCmd('SET', KEYS.ZALO_ACCESS_TOKEN, access_token);
  if (refresh_token) await redisCmd('SET', KEYS.ZALO_REFRESH_TOKEN, refresh_token);
...
(async () => {
  const savedAccess = await redisCmd('GET', KEYS.ZALO_ACCESS_TOKEN);
  const savedRefresh = await redisCmd('GET', KEYS.ZALO_REFRESH_TOKEN);
...
async function setTokensManually(accessToken, refreshToken) {
  ...
  await redisCmd('SET', KEYS.ZALO_ACCESS_TOKEN, accessToken);
  await redisCmd('SET', KEYS.ZALO_REFRESH_TOKEN, refreshToken);
