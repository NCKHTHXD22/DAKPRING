...
const DAK_PRING  = { lat: 15.52933989, lng: 107.61531369 }
const FAR_VIEW   = { lat: 13, lng: 106.5, altitude: 2.6 }
const CLOSE_VIEW = { lat: DAK_PRING.lat, lng: DAK_PRING.lng, altitude: 0.08 }

const MAP_CENTER = [15.52933989, 107.61531369]
const MAP_ZOOM   = 12
  ...
  return L.divIcon({
    ...
    html: `
      <div style="position:relative;width:26px;height:26px;display:flex;align-items:center;justify-content:center">
        <div style="position:absolute;width:26px;height:26px;border-radius:50%;background:${color};opacity:.25;animation:dakpring-pulse 1.8s ease-out infinite"></div>
      ...
      <style>@keyframes dakpring-pulse{0%{transform:scale(.6);opacity:.5}100%{transform:scale(1.6);opacity:0}}</style>
    `,
    iconSize: [26, 26],
    iconAnchor: [13, 13],
    popupAnchor: [0, -16],
...
export default function GlobeHero() {
  ...
  const flyToDakPring = useCallback(() => {
    ...
    >
      ...
      <div className="absolute inset-0 z-0" style={{ transform: 'translateX(-11%)' }}>
        <Globe
          ...
          onGlobeClick={flyToDakPring}
          onPointClick={flyToDakPring}
